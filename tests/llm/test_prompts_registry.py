...
the streamer can edit. These tests pin:
  ...
  - Every editable call site has a non-empty factory + adlib_template
    + at least one slot.
  - resolve_prompt round-trips factory / adlibs / custom modes.
  - Defensive fallback: bad app_settings → factory prompt, never
    empty / broken output to the LLM.
  - Revert clears every override key.
  - Adlib template format errors fall back to factory.
...
def test_every_prompt_has_factory_and_adlib_template():
    """Each registry entry must have non-empty factory text + an
    adlib_template + at least one adlib slot. Catches a half-
    finished registry add."""
    for pd in P.all_prompt_defs():
        assert pd.factory.strip(), f"{pd.call_site}: empty factory"
        assert pd.adlib_template.strip(), f"{pd.call_site}: empty template"
        assert len(pd.adlib_slots) >= 1, f"{pd.call_site}: no slots"


def test_every_adlib_slot_has_default():
...
def test_adlib_template_contains_every_declared_slot_placeholder():
    """Every slot the registry declares must have a matching
    `{slot_name}` placeholder in the adlib_template. Catches
    ...
    for pd in P.all_prompt_defs():
        for slot in pd.adlib_slots:
            ...
            assert placeholder in pd.adlib_template, (
...
def test_adlib_render_substitutes_every_slot(tmp_repo):
    """End-to-end: switch to adlibs mode, save sentinel values,
    confirm the rendered prompt has the values substituted in
    AND no `{slot_name}` placeholders are left dangling."""
    for pd in P.all_prompt_defs():
        P.save_mode(pd.call_site, "adlibs", tmp_repo)
        sentinels = {
            slot.name: f"<<SENTINEL-{slot.name}>>"
            for slot in pd.adlib_slots
        }
        P.save_adlib_values(pd.call_site, sentinels, tmp_repo)
...
# ---- resolve_prompt: adlibs mode ----


def test_resolve_adlibs_with_defaults(tmp_repo):
    """Mode set to adlibs but no values saved → renders with
    every slot's default. Equivalent in shape to factory + the
    injection block, never broken."""
    P.save_mode("insights.talking_points", "adlibs", tmp_repo)
...
def test_resolve_adlibs_with_streamer_overrides(tmp_repo):
...
def test_resolve_adlibs_partial_overrides_fall_back_to_defaults(tmp_repo):
    ...
    rendered = P.resolve_prompt("insights.talking_points", tmp_repo)
    pd = P.get_prompt_def("insights.talking_points")
    avoid_default = next(s.default for s in pd.adlib_slots if s.name == "avoid")
    assert "spicy" in rendered
    assert avoid_default in rendered


def test_resolve_adlibs_corrupt_json_falls_back_to_factory(tmp_repo):
    """If app_settings has malformed JSON for the adlib values
    ...
    rendered = P.resolve_prompt("insights.talking_points", tmp_repo)
    # Adlib mode rendering uses defaults when JSON is bad — so we
    # get an adlib-rendered prompt with all defaults, NOT the
    # plain factory. Either is acceptable; pin which.
    pd = P.get_prompt_def("insights.talking_points")
    assert pd.adlib_slots[0].default in rendered
...
def test_save_adlib_values_filters_unknown_slots(tmp_repo):
    ...
    saved = P.get_adlib_values("insights.talking_points", tmp_repo)
    assert "tone" in saved
    assert "ghost_slot" not in saved


def test_revert_clears_every_override_key(tmp_repo):
    """Revert wipes mode + adlibs + custom in one shot. Idempotent."""
    P.save_mode("insights.talking_points", "custom", tmp_repo)
    P.save_adlib_values(
    ...
    assert tmp_repo.get_app_setting("prompts.insights.talking_points.adlibs")
